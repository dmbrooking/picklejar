...
(function() {
  ...
  $(document).ready(function($) {
    var about_icons, arrows, brand, goToByScroll, htmlbody, links, onScroll, scrollOnClick, showToolTip;
    ...
    goToByScroll = function(dataslide) {
      ...
      while (i < links.length) {
        if (links[i].getAttribute('data-slide') === dataslide) {
          ...
          links[i].parentElement.className = links[i].parentElement.className.split(/\s+/).map(function(val) {
            if (val === 'active') {
              return '';
            } else {
              return val;
    ...
    onScroll = function(event) {
      var i, refElement, scrollPosition;
      scrollPosition = $(document).scrollTop();
      i = 0;
      while (i < links.length) {
        if (links[i].getAttribute('data-slide')) {
          refElement = $('.slide[data-slide="' + links[i].getAttribute('data-slide') + '"]');
          if (refElement.offset().top <= scrollPosition && refElement.position().top + refElement.height() > scrollPosition) {
            links[i].parentElement.className += ' active';
          } else {
            links[i].parentElement.className = links[i].parentElement.className.split(/\s+/).map(function(val) {
              if (val === 'active') {
