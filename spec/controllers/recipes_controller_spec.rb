    ...
    describe '#create' do
      ...
      describe 'with valid params' do
        it 'creates a new Recipe' do
          expect {
            post :create, {:url => Faker::Internet.url('www.epicurious.com')}, valid_session
        ...
        it 'persists the new recipe to the database' do
          ...
          post :create, {:url => url, :title => title}, valid_session

          # confirm that recipe persisted
          persistedRecipes = Recipe.where(url: url, title: title)
          expect(persistedRecipes).to_not be_nil
          expect(persistedRecipes.to_a.length).to eq(1)
          expect(persistedRecipes.to_a[0].url).to eq(url)
          expect(persistedRecipes.to_a[0].title).to eq(title)
        end
